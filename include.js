...
function dateToVar() {
    ...
    t._isValid === true ? (reqdate.full = moment(t, "MM-DD").format("YYYY-MM-DD")) : dateReset();
...
function alertCheck(a) {
    ...
    if (document.getElementById("chkMail").checked) {
        chkMail = 2;
    }
    const alertType = chkMsg + chkMail;
    ltalk.ALERT_TYPE = alertType;
}
// 통합 검증 회로  **************
function roomListFind(isAjax) {
    ...
    list_add.forEach(function (item, index, array) {
        item !== null ? $("#search_result").append('<span class="badge badge-primary">' + item + "</span>" + postposition[index]) : "";
    });
...
function list_mapping() {
    ...
    //  ajax에서 추출하기
    let find_floor_room_result = list_ajax_return.ROOMLIST.filter((c) => c.ROOM_NAME.indexOf(floor_array.location) !== -1);
    let find_floor_meeting_result = list_ajax_return.RESVLIST.filter((c) => c.ROOM_NAME.indexOf(floor_array.location) !== -1);

    // 예약 있는 회의실 찾기
    find_floor_room_result.forEach(function (e, i, array) {
        let _testArr = find_floor_meeting_result.filter((n) => n.ROOM_ID.indexOf(find_floor_room_result[i].ROOM_ID) !== -1);
...
$(document).ready(function () {
    ...
    // 날짜, 시간, 예약명 클릭 시 리셋
    ["#_userDate", "#_userTime", "#room_name", "#floor_input"].forEach(function (e) {
        $(e).focus(function () {
            $(e).val("");
        });
    });
...
function roomfind(resvData) {
    $(resvData.ROOMINFO).each(function (index, value) {
        ...
        ltalk.oRoomInfo = " [" + oCATEGORY_NAME + "] " + oROOM_NAME.replace("<br/>", "") + " (정원 " + oRROOM_LIMIT + "명)";
    });
}
...
function startEndTimeUpdate() {
    reqdate.start = moment(reqdate.full + " " + reqdate.time, "YYYY-MM-DD HH:mm").format("YYYY-MM-DD HH:mm");
...
$.ajax({
    url:
      "https://hook.integromat.com/radx358t7kegbyvjhlkxdkqblxmdqz6i",
    type: "POST",
    data: {
      start: reqdate.start,
      end: reqdate.end,
      title: ltalk.room_title,
      description: ltalk.room_title,
      member: '',
      location: ltalk.oRoomInfo,
      isModify: "N",
